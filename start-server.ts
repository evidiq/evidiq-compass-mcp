...
import { getSignerAddress } from "./lib/compass/report.js";
...
const PUBLIC_BASE_URL = (process.env.PUBLIC_BASE_URL || "https://mcp.evidiq.dev/compass").replace(/\/$/, "");
const SLUG = "compass";

const bypassed = isX402Bypassed();
if (bypassed) {
  console.warn("[compass] X402 GATE BYPASSED — Phase 1 test build. Paid tools answer without payment.");
}

const index = new CompassIndex(
  process.env.COMPASS_DB_PATH || "/data/compass.db",
  process.env.COMPASS_SNAPSHOTS_DIR || "/data/snapshots",
);
await index.ingest();
console.log(
  `[compass] ingested index: ${index.state().agents} agents, ${index.state().services} services, ` +
...
httpServer.listen(PORT, HOST, () => {
  console.log(`[evidiq-compass-mcp] listening at http://${HOST}:${PORT}`);
  console.log(`[evidiq-compass-mcp] Payment Gate: ${bypassed ? "BYPASSED (TEST BUILD)" : "ENFORCED"}`);
  console.log(`[evidiq-compass-mcp] Endpoints: /health, /x402, /skill.md, /mcp`);
});

process.on("unhandledRejection", (reason) => {
  console.error("[evidiq-compass-mcp] unhandled rejection — staying up:", reason);
});
process.on("uncaughtException", (error) => {
  console.error("[evidiq-compass-mcp] uncaught exception — staying up:", error);
});
