...
export const DIGEST_FIELDS: (keyof ReportPayload)[] = [
  "query",
  "basis",
  "snapshotAt",
  "coverage",
...
export function canonicalPayload(payload: ReportPayload): string {
...
export function reportDigest(payload: ReportPayload): string {
...
export function getSignerKey(env: NodeJS.ProcessEnv = process.env): string | null {
  const raw = env.COMPASS_SIGNER_PRIVATE_KEY ?? null;
  if (!raw) return null;
...
export function requireSigner(env: NodeJS.ProcessEnv = process.env): { privateKey: `0x${string}`; address: `0x${string}` } {
  ...
  if (!pk) {
    throw new Error(
      "COMPASS_SIGNER_PRIVATE_KEY missing or invalid — no fallback signing key exists",
    );
  }
