...
export function getX402Config(): X402ServerConfig {
  return {
    ...
    publicBaseUrl: (process.env.PUBLIC_BASE_URL || "https://mcp.evidiq.dev/compass").replace(/\/$/, ""),
    okxCredentials: getOkxCredentials(),
  };
}

/**
 * Phase 1 build/test bypass: when COMPASS_X402_BYPASS=1 or X402_BYPASS=1,
...
export function isX402Bypassed(): boolean {
  return process.env.COMPASS_X402_BYPASS === "1" || process.env.X402_BYPASS === "1";
}
